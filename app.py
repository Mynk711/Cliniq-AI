...
[data-testid="stSidebar"] strong { color: #EFF6FF !important; }
[data-testid="stSidebar"] h1, [data-testid="stSidebar"] h2, [data-testid="stSidebar"] h3 { color: #EFF6FF !important; }
[data-testid="stSidebar"] .stButton > button { background-color: #0D2040 !important; color: #7A9CC4 !important; border: 1px solid #183560 !important; border-radius: 8px; font-size: 11px; padding: 6px 10px; width: 100%; transition: all 0.15s; }
[data-testid="stSidebar"] .stButton > button:hover { border-color: #1AC8D4 !important; color: #1AC8D4 !important; background-color: #041820 !important; }
[data-testid="collapsedControl"] { color: #1AC8D4 !important; background-color: #0A1F3D !important; border: 1px solid #183560 !important; }
...
[data-testid="stMetric"] { background-color: #0D2040 !important; border: 1px solid #183560; border-radius: 10px; padding: 10px 14px; }
[data-testid="stMetricLabel"] { color: #6B9CC4 !important; font-size: 10px !important; letter-spacing: 1px; text-transform: uppercase; }
[data-testid="stMetricLabel"] p { color: #6B9CC4 !important; display: block !important; }
...
[data-baseweb="base-input"] * { -webkit-text-fill-color: #EFF6FF !important; color: #EFF6FF !important; }
[data-baseweb="textarea"] * { -webkit-text-fill-color: #EFF6FF !important; color: #EFF6FF !important; }
[data-baseweb="base-input"] textarea { -webkit-text-fill-color: #EFF6FF !important; color: #EFF6FF !important; }
</style>
""", unsafe_allow_html=True)
...
def render_route_badge(route):
    config = ROUTE_CONFIG.get(route, ROUTE_CONFIG["D"])
    st.markdown(
        f"""<span style='background-color:{config["color"]}22;color:{config["color"]};border:1px solid {config["color"]};padding:2px 10px;border-radius:12px;font-size:11px;font-weight:bold;'>{config["label"]}</span>""",
        unsafe_allow_html=True
    )
...
def process_question(question):
    ...
    with st.chat_message("assistant"):
        ...
        chat_history = build_chat_history()
        result = answer(question, chat_history)
        status.markdown("<span style='color:#1AC8D4;font-size:12px;'>Synthesizing clinical answer...</span>", unsafe_allow_html=True)
        time.sleep(0.3)
        status.empty()
        st.markdown(result["answer"])
        render_route_badge(result["route"])
